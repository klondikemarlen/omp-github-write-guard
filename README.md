...
Approval permits one exact retry; a prior standard external GitHub approval for the same action and target may authorize one boundary-equivalent retry so duplicate gates do not deadlock. Rejection or no UI blocks. An unresolved target blocks. The extension holds one in-memory, target-bound approval until its matching retry; an explicit `OMP_REPOSITORY_BOUNDARY_GUARD_ALLOW_MIXED=1` prefix (or `boundaryOverride: "allow-mixed"` on a direct local `write`/`edit`) allows one resolved external mutation without an Ask. Overrides never allow unresolved, compound, or unknown targets.
